...
const voicesFactory = (syncopation, isMinor, chromaticBass, options) => {
  let voices;
  if (syncopation) {
...
const getVoices = fauxbourdonOptions => {
  ...
  const syncopation = options.addProps.syncopation[0];
  const chromaticBass = options.addProps.chromaticBass[0];
  let voices = voicesFactory(syncopation, isMinor, chromaticBass, options);
